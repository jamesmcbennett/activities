...
def activities():
    ...
    totalactivities = db.execute(
        "SELECT race_name, SUM(ROUND(meters/1000,2)) as totalkilometers FROM activities WHERE user_id=:user_id GROUP BY race_name", user_id=session["user_id"])

    # List all activities by user
    activities = db.execute(
        "SELECT race_id, race_name, meters, time_stamp FROM activities WHERE user_id=:user_id", user_id=session["user_id"])
    ...
    return render_template("activities.html", activities=activities, totalactivities=totalactivities, username=username)
...
def map():
    ...
    return render_template("map.html", totalactivities=totalactivities, username=username)
...
def add():
    ...
    if request.method == "POST":
        ...
        prevKilometers = db.execute("SELECT kilometers FROM activities WHERE id=:user_id",
                    user_id=session["user_id"])

        db.execute("INSERT INTO activities(user_id, race_id, race_name, meters) VALUES (:user_id, :race_id, :race_name, :meters) ",
                   user_id=session["user_id"],
                   race_id=1,
                   race_name=request.form.get("race"),
        ...
        #db.execute("UPDATE activities SET kilometers = kilometers + :amount WHERE id=:user_id",
        ...
        return render_template("add.html", races=[row["race_name"] for row in rows])
...
def races():
    ...
    rows = db.execute("SELECT id, race_name, start, finish, distance, type FROM races")
    races = []
    for row in rows:
        races.append({
            "id": row["id"],
